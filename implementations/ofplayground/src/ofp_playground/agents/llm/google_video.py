"""Google Veo video agent: text-to-video generation."""
...
# Fallback chain: quality → fast
VIDEO_MODELS = [
    "veo-3.1-generate-preview",
    "veo-3.1-fast-generate-preview",
]
MAX_RETRIES_PER_MODEL = 3  # video generation takes 2-6 min each — 3 attempts is plenty
GEMINI_VIDEO_URI_TEMPLATE = "tag:ofp-playground.local,2025:gvideo-{name}"


class GeminiVideoAgent(BasePlaygroundAgent):
    """Video agent that generates videos via the Google Veo API (text-to-video)."""
    ...
    def __init__(
        ...
        speaker_uri = GEMINI_VIDEO_URI_TEMPLATE.format(name=name.lower().replace(" ", "-"))
        super().__init__(
            speaker_uri=speaker_uri,
            name=name,
            service_url=f"local://gvideo-{name.lower()}",
            bus=bus,
    ...
    def _build_manifest(self) -> Manifest:
        return Manifest(
            identification=Identification(
                ...
                role="Google Veo video generation agent",
            ),
            capabilities=[
                Capability(
                    keyphrases=["text-to-video", "video-generation", "veo"],
    ...
    async def _generate_video(self, prompt: str) -> Optional[tuple[Path, str]]:
        ...
        def _call(model: str) -> Optional[Path]:
            ...
            while not operation.done:
                logger.debug("[%s] Veo polling — waiting for video...", self._name)
                time.sleep(10)
            ...
            for attempt in range(1, MAX_RETRIES_PER_MODEL + 1):
                try:
                    ...
                    if path is not None:
                        return path, model
                    logger.warning(
                        "[%s] Veo returned no video from %s (attempt %d/%d)",
                    ...
                    if "503" in err_str or "UNAVAILABLE" in err_str or "400" in err_str or "invalid" in err_str.lower():
                        ...
                        break
                    logger.warning(
                        "[%s] Veo error on %s attempt %d/%d: %s",
        ...
        logger.error("[%s] All Veo models exhausted with no result", self._name)
        return None
    ...
    async def _handle_grant_floor(self) -> None:
        ...
        try:
            ...
            if prompt:
                logger.info("[%s] Generating Veo video: %s", self._name, prompt[:80])
