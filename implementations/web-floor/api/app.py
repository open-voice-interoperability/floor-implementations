...
def proxy_send():
	...
	if not allowed:
		return jsonify({"ok": False,"status": 0,"statusText": "BLOCKED_TARGET","error": reason,}), 200
	outbound_body = json.dumps(payload).encode("utf-8")
	outbound = Request(target_url,data=outbound_body,method="POST",headers={"Content-Type": "application/json","Accept": "application/json","User-Agent": "web-floor-flask-gateway/0.1",},)
	try:
		...
		return jsonify({"ok": False,"status": 0,"statusText": "REQUEST_ERROR","error": str(error.reason),}), 200
	except Exception as error:
		return jsonify({"ok": False,"status": 0,"statusText": "REQUEST_ERROR","error": str(error),}), 200
	try:
		parsed_json = json.loads(raw_text)
	except json.JSONDecodeError:
		parsed_json = None
	return jsonify({"ok": 200 <= status < 300,"status": status,"statusText": status_text,"text": raw_text,"json": parsed_json,}), 200

@app.route("/health", methods=["GET"])
def health():
	return jsonify({"status": "healthy","service": "web-floor-flask-gateway","port": int(os.environ.get("PORT", "8090")),})
